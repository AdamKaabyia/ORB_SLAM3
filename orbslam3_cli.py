...
class ORBSlam3CLI:
    ...
    def compare_upstream_build_and_run(self):
        ...
        # Use an isolated (relative) results directory per run to avoid mixing with old trajectories
        results_dir_name = f"results_run_{int(time.time())}"
        (self.workspace / results_dir_name).mkdir(exist_ok=True)
        env["RESULTS_DIR"] = results_dir_name
    ...
    def full_one_shot_pipeline(self):
        ...
        env["PYTHONUNBUFFERED"] = "1"
        env["RICH_FORCE_TERMINAL"] = "1"
        # Use dedicated (relative) per-run results directory for clean aggregation
        results_dir_name = f"results_run_{int(time.time())}"
        (self.workspace / results_dir_name).mkdir(exist_ok=True)
        env["RESULTS_DIR"] = results_dir_name
        cmd = (
